...
def collate_and_to_numpy(batch):
...
def load_programs(dsl: DSL):
    ...
    # StdoutLogger.log(
    #     "Data Loader",
    #     "Total programs with length <= {}: {}".format(
    #         Config.data_max_program_length, len(program_list)
    #     ),
    # )
...
class ProgramDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        prog_sufix = torch.tensor(
            (self.max_program_len - program_len - 1) * [self.pad_token],
            device=self.device,
            dtype=torch.long,
        )
        ...
        a_h_expanded = np.ones((a_h.shape[0], self.max_demo_length), dtype=int) * (
            self.num_agent_actions - 1
        )
        s_h_expanded = np.zeros(
            (s_h.shape[0], self.max_demo_length, *s_h.shape[2:]), dtype=bool
        )
        ...
        for i in range(a_h_len.shape[0]):
            ...
            s_h_expanded[i, a_h_len[i] + 1 :] = s_h[i, a_h_len[i]] * (
                self.max_demo_length - a_h_len[i] + 1
            )
...
def make_dataloaders(dsl: DSL, device: torch.device = 'cpu'):
    # StdoutLogger.log(
    #     "Data Loader",
    ...
    split_idx2 = int(
        (Config.data_ratio_train + Config.data_ratio_val) * len(program_list)
    )
