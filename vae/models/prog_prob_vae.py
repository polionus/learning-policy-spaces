...
from ..utils import init_gru, init_gru_cell, init_linear, init_conv2d, GRU
from .base_vae import BaseVAE, ModelOutput, STATE_SHAPE
...
def make_progs_probs_vae(progs_teacher_enforcing: bool, a_h_teacher_enforcing: bool):
    class ProgProbVAE(BaseVAE):
        ...
        def __init__(self, dsl: DSL, hidden_size: int, key):
            ...
            ## We already have a state encoder in the base vae file. 
            ## We can reuse it here. 

            ## I would like to simplify the logic here as well. 
            self.problem_encoder = self.state_encoder = eqx.nn.Sequential(
            [
        ...
        def encode(self, progs: jax.Array, progs_mask: jax.Array, problems: jax.Array):
            ...
            enc_problems = eqx.filter_vmap(problems)
            enc_hidden_state = jnp.concatenate((enc_hidden_state, enc_problems), axis = -1)
...
LeapsVAE = make_progs_probs_vae(TrainConfig.prog_teacher_enforcing, TrainConfig.a_h_teacher_enforcing)
