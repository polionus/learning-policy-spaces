...
def leaps_loss_fn(params, batch: Batch, static) -> Tuple[jax.Array, NamedTuple]:
    ...
    a_h = jnp.reshape(batch.a_h, (-1, batch.a_h.shape[-1]))#.view(-1, a_h.shape[-1])
    a_h_masks = jnp.reshape(batch.a_h_masks, (-1, batch.a_h_masks.shape[-1]))#.view(-1, a_h_masks.shape[-1])
    ...
    # Skip first token in ground truth sequences
    progs = batch.progs[:, 1:]
    progs_masks = batch.progs_masks[:, 1:]
    ...
    return total_loss, Aux(latent_loss=latent_loss, progs_loss=progs_loss, a_h_loss=a_h_loss)
...
def probs_loss_fn(params, batch, static) -> Tuple[jax.Array, NamedTuple]:
    s_h, a_h, a_h_masks, progs, progs_masks = batch
    # The model takes in these parameters, and outputs: Teacher enforcing is enabled by default, and so was also enabled for us.
    model = eqx.combine(params, static)
    output = model(
    ...
    ## TODO: This needs to be modified. it depends on the __call__ function of the model.
    (mu,
        sigma,
        _,
        _,
