...
### TODO: Just change the types here to jax
...
class CheckerState(object):
    ...
    def __copy__(self):
        new_state = CheckerState(
            self.state,
            self.next_mandatory,
            self.i_need_else_stack_pos,
            self.to_close_stack_pos,
            self.c_deep,
            self.next_actblock_open,
        )
...
class PySyntaxChecker(object):
    ...
    def __init__(self, T2I: dict, device: torch.device, only_structure: bool = False):
        ...
        self.need_else = {T2I["IF"]: False, T2I["IFELSE"]: True}
    ...
    def get_initial_checker_state(self):
        return CheckerState(STATE_MANDATORY_NEXT, self.vocab.def_tkn, -1, -1, 0, -1)

    def get_initial_checker_state2(self):
        return CheckerState(STATE_MANDATORY_NEXT, self.vocab.m_open_tkn, -1, -1, 0, -1)
