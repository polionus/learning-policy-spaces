...
def _get_node_from_token(token: str) -> Node:
    ...
    if token == "rightIsClear":
        return RightIsClear()
    if token == "markersPresent":
        return MarkersPresent()
    if token == "noMarkersPresent":
        return NoMarkersPresent()
...
def _find_close_token(
    token_list: list[str], character: str, start_index: int = 0
) -> int:
    ...
    if token_list[0] == "DEF":
        ...
        if i_end == len(token_list) - 1:
            ...
            return Conjunction.new(
                If.new(c, i), _str_list_to_node(token_list[i_end + 1 :])
            )
        ...
        if e_end == len(token_list) - 1:
            ...
            return Conjunction.new(
                ITE.new(c, i, e), _str_list_to_node(token_list[e_end + 1 :])
            )
        ...
        if w_end == len(token_list) - 1:
            ...
            return Conjunction.new(
                While.new(c, w), _str_list_to_node(token_list[w_end + 1 :])
            )
        ...
        if r_end == len(token_list) - 1:
            ...
            return Conjunction.new(
                Repeat.new(n, r), _str_list_to_node(token_list[r_end + 1 :])
            )
