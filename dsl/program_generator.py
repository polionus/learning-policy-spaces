...
class ProgramGenerator:
    ...
    def _fill_children(
        self, node: Node, current_depth: int = 1, current_sequential_length: int = 0
    ) -> None:
        ...
        for i, child_type in enumerate(node.get_children_types()):
            ...
            for child_type in child_probs:
                ...
                if current_depth >= self.max_depth and child_type.get_node_depth() > 0:
                    child_probs[child_type] = 0
            if (
                issubclass(type(node), Conjunction)
                and current_sequential_length + 1 >= self.max_sequential_length
            ):
            ...
            if child.get_number_children() > 0:
                if issubclass(type(node), Conjunction):
                    self._fill_children(
                        child_instance,
                        current_depth + child.get_node_depth(),
                        current_sequential_length + 1,
                    )
                else:
                    self._fill_children(
                        child_instance, current_depth + child.get_node_depth(), 1
                    )

            elif child == ConstIntNode:
                child_instance.value = self.rng.choice(
                    ProgramGenerator.valid_int_values
                )
            elif child == ConstBoolNode:
                child_instance.value = self.rng.choice(
                    ProgramGenerator.valid_bool_values
                )
    ...
    def generate_program(self) -> Program:
        while True:
            ...
            if (
                program.get_size() <= self.max_program_size
                and len(self.dsl.parse_node_to_int(program)) <= self.max_program_length
            ):
                break
        return program
    ...
    def generate_demos(
        ...
        num_demos: int,
        max_demo_length: int,
        cover_all_branches: bool = True,
        timeout: int = 250,
    ) -> tuple[np.ndarray, np.ndarray]:
        action_nodes = set(
            [
                n
                for n in prog.get_all_nodes()
