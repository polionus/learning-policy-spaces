...
def main():
    ...
    state, sequence_mask = syntax_checker.get_sequence_mask(
        initial_state, sample_program
    )
   
    raise Exception('You have not finished implementing this to jax.')
    # for idx, token in enumerate(sample_program):
    #     valid_tokens = torch.where(sequence_mask[idx] == 0)[0]
    #     valid_tokens = [
    #         dsl.i2t[tkn.detach().cpu().numpy().tolist()] for tkn in valid_tokens
    #     ]
