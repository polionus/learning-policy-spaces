...
class BottomUpSearch:
    ...
    def grow_node(
        self, node: Node, plist: list[Node], new_plist: list[Node], size: int
    ):
        types = node.get_children_types()
        possible_children = []
        for child_type in types:
            possible_children.append(
                [p for p in plist if p.__class__ in child_type.__subclasses__()]
            )
    ...
    def grow(self, plist: list[Node], production: DSL, size: int):
        ...
        for op in [
            obj
            for obj in production.nodes
            if obj.__class__ not in TerminalNode.__subclasses__()
        ]:
    ...
    def synthesize(self, data: Data, production: DSL, bound) -> tuple[Node, int]:
        ...
        plist = [
            obj
            for obj in production.nodes
            if obj.__class__ in TerminalNode.__subclasses__()
        ]
