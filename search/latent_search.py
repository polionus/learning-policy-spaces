...
def execute_program(
    ...
    if not program.is_complete():
        ...
        tds = TopDownSearch()
        tds_result = tds.synthesize(
            program, dsl, task_envs, Config.datagen_sketch_iterations
        )
...
class LatentSearch:
    ...
    def __init__(self, model: BaseVAE, task_cls: type[Task], dsl: DSL):
       ...
       ### NOTE: The search algorithm determines the metadata and config of the experiment.
    ...
    def execute_population(
        ...
        programs_str = [
            self.dsl.parse_int_to_str(prog_tokens) for prog_tokens in programs_tokens
        ]
        ...
        if Config.multiprocessing_active:
            ...
            results = [
                execute_program(p, self.task_envs, self.dsl) for p in programs_str
            ]
        ...
        for p, num_eval, r in results:
            ...
            if r > self.best_reward:
                ...
                #     t = time.time() - self.start_time
                #     f.write(
                #         f"{t},{self.num_evaluations},{self.best_reward},{self.best_program}\n"
                #     )
    ...
    def search(self) -> tuple[str, bool, int]:
        ...
        for iteration in range(1, self.number_iterations + 1):
            ...
            if counter_for_restart >= self.restart_timeout and self.restart_timeout > 0:
                ...
                ###TODO: What is different between this and the commented out one in CEM? They should be the same, but they behave
                ### differently
