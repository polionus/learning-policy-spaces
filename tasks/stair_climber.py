...
from karel.world import WorldState, make_world_state, STATE_TABLE 
...
class StairClimber(Task):
    ...
    def generate_state(self):
        ...
        marker_position_index = self.rng.randint(
            initial_position_index + 1, len(on_stair_positions)
        )
    ...
    def get_reward(self, world_state: WorldState):

        terminated = False
        reward = 0

        karel_pos = world_state.hero_pos
        ...
        if (
            karel_pos[0] == self.marker_position[0]
            and karel_pos[1] == self.marker_position[1]
        ):
            terminated = True
