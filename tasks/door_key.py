...
class DoorKey(Task):
    ...
    def generate_state(self):
        ...
        self.key_cell = (
            self.rng.randint(1, self.env_height - 1),
            self.rng.randint(1, wall_column),
        )
        self.end_marker_cell = (
            self.rng.randint(1, self.env_height - 1),
            self.rng.randint(wall_column + 1, self.env_width - 1),
        )
    ...
    def get_reward(self, world_state: World):
        ...
        if self.door_locked:
            ...
            if num_markers > 1:
                ...
                if (
                    world_state.markers_grid[
                        self.end_marker_cell[0], self.end_marker_cell[1]
                    ]
                    == 2
                ):
                    terminated = True
