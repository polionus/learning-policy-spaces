...
class CleanHouse(Task):
    ...
    def generate_state(self):
        ...
        agent_pos = (1, 13)
        hardcoded_invalid_marker_locations = set(
            [(1, 13), (2, 12), (3, 10), (4, 11), (5, 11), (6, 10)]
        )
        ...
        for y1 in range(self.env_height):
            for x1 in range(self.env_width):
                if state[y1, x1, 4]:
                    if y1 - 1 > 0 and not state[y1 - 1, x1, 4]:
                        expected_marker_positions.add((y1 - 1, x1))
                    if y1 + 1 < self.env_height - 1 and not state[y1 + 1, x1, 4]:
                        expected_marker_positions.add((y1 + 1, x1))
                    if x1 - 1 > 0 and not state[y1, x1 - 1, 4]:
                        expected_marker_positions.add((y1, x1 - 1))
                    if x1 + 1 < self.env_width - 1 and not state[y1, x1 + 1, 4]:
                        expected_marker_positions.add((y1, x1 + 1))

        possible_marker_locations = list(
            expected_marker_positions - hardcoded_invalid_marker_locations
        )
    ...
    def get_reward(self, world_state: World):
        ...
        ) / self.initial_number_of_markers
